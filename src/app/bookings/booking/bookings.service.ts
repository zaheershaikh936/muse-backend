...
export class BookingsService {
  ...
  async updateBookingStatus(paymentUrl: string, body: any) {
    ...
    if (body.status !== 'cancelled')
      body.uniqueUrl = await encryptBookingData(
        decryptData[0],
        decryptData[1],
        decryptData[2],
      );
    const data = await this.bookingModel.findByIdAndUpdate(
      { _id: new ObjectId(decryptData[0]) },
      { $set: { ...body } },
      { new: true },
    );
    return {
      ...
      refundId: data?.payment?.refundId,
    };
  }
  ...
  async findOneByPaymentUrl(paymentUrl: string) {
    ...
    return this.bookingModel
      .findById(
        { _id: new ObjectId(decryptData[0]), status: 'paid', isPaid: true },
        { mentor: 1, user: 1, booking: 1, status: 1, uniqueUrl: 1, amount: 1 },
      )
      .lean()
      .exec();
  }
  ...
  async findOneById(id: string): Promise<any> {
    const data = await this.bookingModel
      .findById(id, { paymentDate: 0, uniqueUrl: 0, updatedAt: 0, isPaid: 0 })
      .lean();
  ...
  async findBookingById(_id: string) {
    return this.bookingModel
      .findById({ _id }, { 'booking.endTime': 1, _id: 0 })
      .lean()
      .exec();
  }
}
