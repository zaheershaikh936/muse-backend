...
export class AvailabilityService {
  ...
  findAvailability(id: string) {
    return this.availabilityModel
      .findOne({ mentorId: new ObjectId(id) }, { userId: 1 })
      .lean()
      .exec();
  }
  ...
  findAllAvailability(id: string) {
    return this.availabilityModel
      .findOne({ userId: new ObjectId(id) }, { _id: 1, availability: 1 })
      .lean()
      .exec();
  }
  ...
  findMentorAvailability(id: string) {
    return this.availabilityModel
      .findOne({ userId: new ObjectId(id) }, { availability: 1 })
      .lean()
      .exec();
  }
  ...
  getAvailableSlots(availability: any[], bookings: any[]) {
    ...
    availability.forEach((slot) => {
      ...
      while (current < end) {
        ...
        if (!isBooked) {
          slots.push({
            start_time_string: current.toLocaleTimeString([], {
              hour: '2-digit',
              minute: '2-digit',
            }),
            end_time_string: nextSlot.toLocaleTimeString([], {
              hour: '2-digit',
              minute: '2-digit',
            }),
